...
class ListManager:
			...
			for event in data:
				...
				if tmp_half == half:
					...
					if "confidence" in event:
						tmp_confidence = event["confidence"]
					event_list.append(Event(tmp_label, tmp_half, tmp_time, tmp_team, tmp_position, tmp_confidence))
		return event_list
	...
	def save_file(self, path, half):
		...
		for event in final_list:
			...
			tmp_dict["team"] = str("null")
			tmp_dict["confidence"] = str(event.confidence)
